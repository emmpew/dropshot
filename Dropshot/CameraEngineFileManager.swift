...
public class CameraEngineFileManager {
    ...
    public class func removeItemAtPath(_ path: String) {
    ...
    public class func removeURL(_ url: URL) {
        let filemanager = FileManager.default
        if filemanager.fileExists(atPath: url.path) {
            do {
                ...
                print("[Camera engine] Error remove path :\(url)")
            }
        }
    }
    ...
    private class func appendPath(_ rootPath: String, pathFile: String) -> String {
        ...
        removeItemAtPath(destinationPath)
    ...
    public class func documentPath(_ file: String) -> URL? {
        if let path = documentPath() {
            return URL(fileURLWithPath: appendPath(path, pathFile: file))
        }
        return nil
    }
    
    public class func temporaryPath(_ file: String) -> URL? {
        return URL(fileURLWithPath: appendPath(temporaryPath(), pathFile: file))
    }
}
